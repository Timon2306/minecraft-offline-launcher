...
function buildVersionsList(manifest) {
  ...
  const allowedPacks = (config.allowedCristalixPacks || ['everyrage', 'skyvoid', 'magica', 'technomagic', 'galax', 'divinepvp']);
  ...
  try {
    ...
    if (fs.existsSync(cristaloxDir)) {
      ...
      items.forEach(item => {
        if (item.isDirectory()) {
          // Определяем сборку Кристаликса по имени из списка allowedCristalixPacks
          const lowerName = item.name.toLowerCase();
          if (allowedPacks.some(pack => lowerName === pack || lowerName.includes(pack) || pack.includes(lowerName))) {
