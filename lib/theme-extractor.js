...
async function extractVersionTheme(gameDir, versionId) {
  try {
    ...
    // Разрешаем базовую версию (например, Cuboid-Outpost -> Forge -> 1.16.5)
    const baseVersionId = resolveBaseVersion(gameDir, versionId);

    // Шаг 1: Пробуем найти настоящие ассеты через assets index (для современных версий 1.8+)
    ...
    // Шаг 2: Если панорамы нет, пробуем извлечь из JAR (для 1.7.10 и модовых версий)
      ...
      if (jarPath) {
        ...
        for (const file of panoramaFiles) {
          ...
          if (entry) {
            ...
            if (data.length > 1000) { // Игнорируем пустые плейсхолдеры
    ...
    // Шаг 3: Если и это не помогло, делаем интеллектуальный фоллбек
    if (result.panorama.length === 0) {
      if (shouldHavePanorama(versionId)) {
        ...
        result.hasJar = false; // Статичный режим
      } else {
    ...
    // Резервные копии базовых widgets/logo
